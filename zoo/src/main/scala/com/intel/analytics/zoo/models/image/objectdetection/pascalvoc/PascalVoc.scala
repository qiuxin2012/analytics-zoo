...
package com.intel.analytics.zoo.models.image.objectdetection.pascalvoc
...
object PascalVoc {
  ...
  def loadAnnotation(path: String, labelMap: Map[String, Float]): RoiLabel = {
    ...
    while (ix < objs.length) {
      ...
      val dfExist = (obj \ "difficult").text.isEmpty
      difficults(ix) = if (dfExist) 0f else (obj \ "difficult").text.toFloat
      ix += 1
    }
